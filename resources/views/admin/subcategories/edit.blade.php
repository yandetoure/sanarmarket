...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <h1 class="text-2xl font-display font-bold text-slate-900">Modifier la Sous-catégorie</h1>
        </div>
            ...
            <form action="{{ route('admin.subcategories.update', $subcategory) }}" method="POST" class="space-y-6">
                ...
                <div class="form-control">
                    ...
                    <textarea name="description" rows="4"
                        class="textarea textarea-bordered rounded-xl focus:ring-2 focus:ring-primary-500 @error('description') textarea-error @enderror">{{ old('description', $subcategory->description) }}</textarea>
                    @error('description')
                        <span class="text-error text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
