...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center space-x-4 mb-8">
        ...
        <h1 class="text-2xl font-display font-bold text-slate-900">Modifier l'utilisateur</h1>
    </div>
    ...
    <div class="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/50">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div class="form-control">
                    <label class="label font-bold text-slate-700">Nouveau mot de passe <span class="text-slate-400 font-normal">(optionnel)</span></label>
                    <input type="password" name="password" 
                           class="input input-bordered rounded-xl focus:ring-2 focus:ring-primary-500 @error('password') input-error @enderror">
                    @error('password')
                        <span class="text-error text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-control">
                    ...
                    <input type="password" name="password_confirmation" 
                           class="input input-bordered rounded-xl focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
