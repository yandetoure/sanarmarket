...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <h1 class="text-2xl font-display font-bold text-slate-900">Modifier l'Article</h1>
        </div>

        <div class="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/50">
            <form action="{{ route('admin.campus-spotlight.update', $campusSpotlight) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="form-control">
                    ...
                    <input type="text" name="title" value="{{ old('title', $campusSpotlight->title) }}"
                ...
                <div class="form-control">
                    ...
                    <select name="type"
                        ...
                        @foreach(['news' => 'Actualité', 'event' => 'Événement', 'info' => 'Information'] as $key => $label)
                            <option value="{{ $key }}" {{ old('type', $campusSpotlight->type) == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-control">
                    ...
                    <textarea name="content" rows="6"
                        ...
                        required>{{ old('content', $campusSpotlight->content) }}</textarea>
                    @error('content')
                        <span class="text-error text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
