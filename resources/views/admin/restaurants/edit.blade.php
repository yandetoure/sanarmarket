...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <h1 class="text-2xl font-display font-bold text-slate-900">Modifier le Restaurant</h1>
        </div>
            ...
            <form action="{{ route('admin.restaurants.update', $restaurant) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="form-control">
                    ...
                    <textarea name="description" rows="4"
                        class="textarea textarea-bordered rounded-xl focus:ring-2 focus:ring-primary-500 @error('description') textarea-error @enderror">{{ old('description', $restaurant->description) }}</textarea>
                    @error('description')
                        <span class="text-error text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="pt-6 flex justify-end space-x-4">
                    <a href="{{ route('admin.restaurants.show', $restaurant) }}"
                        class="btn btn-ghost rounded-xl">Annuler</a>
