...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <h1 class="text-2xl font-display font-bold text-slate-900">Modifier la Boutique</h1>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/50">
            <form action="{{ route('admin.boutiques.update', $boutique) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="form-control">
                    ...
                    <select name="user_id"
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $boutique->user_id) == $user->id ? 'selected' : '' }}>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <!-- Nom -->
                    <div class="form-control">
                        <label class="label font-bold text-slate-700">Nom de la boutique</label>
                        <input type="text" name="name" value="{{ old('name', $boutique->name) }}"
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" name="phone" value="{{ old('phone', $boutique->phone) }}"
                ...
                <div class="form-control">
                    ...
                    <textarea name="description" rows="4"
                        class="textarea textarea-bordered rounded-xl focus:ring-2 focus:ring-primary-500 @error('description') textarea-error @enderror">{{ old('description', $boutique->description) }}</textarea>
                    @error('description')
                        <span class="text-error text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-control">
                    ...
                    <input type="text" name="address" value="{{ old('address', $boutique->address) }}"
                ...
                <div class="pt-6 flex justify-end space-x-4">
                    <a href="{{ route('admin.boutiques.show', $boutique) }}" class="btn btn-ghost rounded-xl">Annuler</a>
                    <button type="submit" class="btn btn-primary rounded-xl px-8">Enregistrer les modifications</button>
                </div>
            </form>
        </div>
    </div>
