        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-12 text-center" data-aos="fade-down">
                <h1 class="text-3xl md:text-5xl font-display font-black text-slate-900 leading-tight">
                    Créez un <span class="text-gradient">Événement</span> mémorable
                </h1>
                <p class="text-slate-500 font-medium mt-4 text-lg">Donnez de la visibilité à vos rassemblements sur tout le
                    campus.</p>
            </div>
            ...
            <div class="glass p-10 md:p-12 rounded-[3rem] border-white/60 shadow-2xl shadow-slate-200/50"
                ...
                <form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                    ...
                    <div class="space-y-8">
                        <div>
                            <label for="title"
                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-3">Titre de
                                l'événement *</label>
                            <input type="text" id="title" name="title" required value="{{ old('title') }}"
                                ...
                                placeholder="Ex: Conférence sur l'IA - Amphi A">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="6" required
                                ...
                                placeholder="Décrivez le programme, les intervenants et les informations pratiques..."></textarea>
                            @error('description') <p class="mt-2 text-xs text-red-500 font-bold">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid md:grid-cols-2 gap-8">
                            <div>
                                <label for="start_date"
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-3">Date
                                ...
                                @error('start_date') <p class="mt-2 text-xs text-red-500 font-bold">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label for="end_date"
                                ...
                                <input type="datetime-local" id="end_date" name="end_date" value="{{ old('end_date') }}"
                                    class="w-full glass-light p-5 rounded-2xl border-white/50 focus:border-primary-400 focus:ring-0 text-slate-900 font-bold transition-all">
                                @error('end_date') <p class="mt-2 text-xs text-red-500 font-bold">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="location" name="location" value="{{ old('location') }}"
                                ...
                                placeholder="Ex: Terrasse du Restau 2, Salle 105...">
                            @error('location') <p class="mt-2 text-xs text-red-500 font-bold">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label
                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-3">Affiche
                            ...
                            <div class="relative group">
                                ...
                                <div
                                    ...
                                    <p class="text-sm font-black text-slate-900">Cliquez pour ajouter l'affiche</p>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Format
                                        JPEG, PNG autorisés</p>
                                </div>
                            </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4 pt-6">
                        <x-button type="submit" variant="primary" size="lg"
                            class="flex-1 shadow-2xl shadow-primary-500/30">Créer mon événement</x-button>
                        <x-button href="{{ route('events.index') }}" variant="secondary" size="lg"
