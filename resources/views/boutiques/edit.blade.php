                ...
                <form method="POST" action="{{ route('boutiques.update', $boutique) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description"
                                  ...
                                  placeholder="Décrivez votre boutique, vos produits, vos services...">{{ old('description', $boutique->description) }}</textarea>
                        @error('description')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select id="status"
                            ...
                            <option value="draft" {{ old('status', $boutique->status) == 'draft' ? 'selected' : '' }}>Brouillon</option>
                            <option value="pending" {{ old('status', $boutique->status) == 'pending' ? 'selected' : '' }}>En attente</option>
                            <option value="active" {{ old('status', $boutique->status) == 'active' ? 'selected' : '' }}>Active</option>
                        </select>
                        @error('status')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        @if($boutique->cover_image)
                            <div style="margin-bottom:12px;">
                                <img src="{{ asset('storage/' . $boutique->cover_image) }}" alt="{{ $boutique->name }}" class="current-image">
                        ...
                        @error('cover_image')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                        <p class="form-help">Image principale de votre boutique (JPEG, PNG, JPG, GIF - Max 2MB). Laisser vide pour conserver l'image actuelle.</p>
                    </div>
                        ...
                        <div style="display:grid;grid-template-columns:1fr;gap:12px;">
                            ...
                            <div>
                                ...
                                <input type="url"
                                       ...
                                       value="{{ old('social_links.twitter', $boutique->social_links['twitter'] ?? '') }}"
                                       class="form-input"
                    ...
                    <div style="display:flex;gap:12px;margin-top:32px;padding-top:24px;border-top:1px solid rgba(15,23,42,0.1);">
                        <a href="{{ route('boutiques.show', $boutique) }}" class="btn btn-secondary" style="flex:1;text-align:center;">
                            Annuler
                        </a>
