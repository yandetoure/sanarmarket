...
<section class="bg-gray-100 min-h-screen">
    <!-- Photo de couverture -->
    <div class="relative w-full h-64 overflow-hidden bg-gradient-to-br from-blue-500 via-purple-600 to-pink-500">
        ...
        @else
            <div class="w-full h-full flex items-center justify-center">
                <div class="text-white text-center">
                    <h2 class="text-5xl font-bold mb-2">{{ $group->name }}</h2>
                </div>
            </div>
        @endif
    </div>
            ...
            <div class="relative -mt-20 mb-4">
                <div class="flex items-end gap-4">
                    ...
                    <div class="relative inline-block">
                        <div class="h-32 w-32 rounded-full bg-gradient-to-br from-blue-600 to-purple-600 border-4 border-white shadow-lg flex items-center justify-center flex-shrink-0">
                            <span class="text-white text-4xl font-bold">{{ strtoupper(substr($group->name, 0, 1)) }}</span>
                        </div>
                    </div>
                ...
                @auth
                    @if($isOwner)
                        <a href="{{ route('forum.groups.edit', $group) }}"
                           class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">
                            <i data-lucide="settings" class="h-4 w-4"></i>
                            Configurer
                        </a>
                    @endif
                    @if($isBanned)
                        <span class="inline-flex items-center gap-2 rounded-lg border border-red-500 px-6 py-2.5 text-sm font-semibold text-red-600">
                            <i data-lucide="shield-alert" class="h-4 w-4"></i>
                            Vous êtes banni
                        </span>
                    @elseif($isMember)
                        <a href="{{ route('forum.create') }}?group={{ $group->id }}"
                           class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 shadow-sm">
                            <i data-lucide="message-circle-plus" class="h-4 w-4"></i>
                            Créer un sujet
                        </a>
                        <span class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-semibold text-gray-700">
                    ...
                    @endif
                    <button class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">
            ...
            <div class="border-t border-gray-200 pt-0">
                <nav class="flex items-center gap-1 -mb-px">
                    <a href="{{ route('forum.groups.show', $group) }}" 
                       class="px-4 py-3 text-sm font-semibold border-b-2 border-blue-600 text-blue-600 transition">
                        Publications
                    </a>
                    <a href="#about" 
                       class="px-4 py-3 text-sm font-medium text-gray-600 hover:text-gray-900 transition">
                        À propos
                    </a>
                    <a href="#members" 
                       class="px-4 py-3 text-sm font-medium text-gray-600 hover:text-gray-900 transition">
